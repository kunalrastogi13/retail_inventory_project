...
with engine.begin() as conn:
    ...
    # clear existing rows for this model_version & as_of_date
    model_version = "v1_stockout_rf"

    # we use the max(date) in df_scoring as 'as_of_date' snapshot
    as_of_date = df_scoring["date"].max()

    print(f"🧽 Deleting old predictions for {model_version} on {as_of_date} ...")
    conn.execute(
        text("""
            DELETE FROM retail.fact_stockout_risk
            WHERE as_of_date = :as_of_date
              AND model_version = :model_version;
        """),
        {"as_of_date": as_of_date, "model_version": model_version}
    )
