...
# 2. Load product IDs from CSV (same file used for dim_product)
df = pd.read_csv("/app/data/global_superstore.csv", encoding="latin1")
product_ids = df["Product ID"].unique().tolist()  # uses original CSV column name
...
def generate_inventory_event():
    ...
    change_qty = random.choice([
        random.randint(1, 10),   # restock
        -random.randint(1, 5),   # consumption
    ])
